...
def model_train_validate_test(train_df, dev_df, test_df, embeddings_file, vocab_file, target_dir, mode,
    ...
    if(embeddings_file is not None):
        ...
        embeddings = None
    model = ABCNN(embeddings=embeddings, num_labels=num_labels, num_layer=1, linear_size=300, max_length=max_length, device=device).to(device)
...
def model_load_test(test_df, vocab_file, embeddings_file, pretrained_file, 
                    test_prediction_dir, test_prediction_name, mode, num_labels=2, max_length=64, gpu_index=0, batch_size=128):
    ...
    test_loader = DataLoader(test_data, shuffle=False, batch_size=batch_size)
    print("\t* Building model...")
    model = ABCNN(embeddings, num_labels=num_labels, num_layer=1, linear_size=300, max_length=max_length, device=device).to(device)
    model.load_state_dict(checkpoint["model"])
